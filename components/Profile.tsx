...
const Profile: React.FC<ProfileProps> = ({ user, trips, milestones, rankings }) => {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-300 pb-8 pt-4">
      <div className="glass p-8 rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden text-center">
        <div className="absolute top-0 right-0 p-8 opacity-10">
          <i className="fa-solid fa-award text-8xl text-emerald-500"></i>
        </div>
        <div className="w-24 h-24 rounded-3xl bg-slate-900 dark:bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-4xl mx-auto mb-4 shadow-xl border border-emerald-500/20">
      ...
      {resolvedMilestones.length > 0 && (
        <div className="space-y-4">
          <div className="flex justify-between items-center ml-1">
            <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Dynamic Achievements</h3>
            <span className="text-[9px] font-bold text-emerald-500 uppercase">Tracked via AI</span>
          </div>
          <div className="space-y-4">
            {resolvedMilestones.map((m) => (
              <div key={m.id} className="glass p-6 rounded-[2rem] border-white/5 shadow-lg group transition-all hover:bg-white/90 dark:hover:bg-slate-800/90">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-slate-900 dark:bg-slate-800 text-emerald-500 flex items-center justify-center text-2xl shadow-xl flex-shrink-0">
            ...
            {resolvedRankings.map((r, i) => (
              <div key={i} className={`flex items-center justify-between p-5 border-b border-slate-50 dark:border-slate-800/50 last:border-none ${r.isUser ? 'bg-emerald-500/10' : 'bg-transparent'}`}>
                <div className="flex items-center gap-4">
                  <span className={`text-[10px] font-black w-6 ${r.rank <= 3 ? 'text-amber-500' : 'text-slate-500'}`}>#{r.rank}</span>
                  ...
                  <span className={`text-sm tracking-tight ${r.isUser ? 'font-black text-slate-800 dark:text-white' : 'font-bold text-slate-600 dark:text-slate-400'}`}>{r.name}</span>
                </div>
